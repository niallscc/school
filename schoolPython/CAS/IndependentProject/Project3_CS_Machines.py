    ...
    def change(self):
        if self.can_change:
            if will_happen(self.prob_speed_change):
                self.direction = WindSpeed[random.randint(0, 3)]
            if will_happen(self.prob_dir_change):
                self.direction = Direction[random.randint(0, 4)]
...
class Board:
    ...
    def __init__(self, width=40, height=40, configuration=BoardType.combo, wind=Wind(), has_roads=False):
    ...
    def build_board(self):
        ...
        if self.board_type == BoardType.forest:
            ...
            for i in range(0, int((w * h) / 2)):
                print 
                self.board[random.randint(0, w - 1)][random.randint(0, h - 1)] = copy.deepcopy(TerrainType.Grass)
        else:
            for i in range(0, (w * h)):
                self.board[random.randint(0, w)][random.randint(0, h)] = copy.deepcopy(TerrainType[random.randint(0, 1)])
        if self.has_roads:
            self.build_road()
...
def will_happen(prob):
    r = [0 for i in range(0,100)]
    # print len(r)
    #r = np.zeros(100, dtype=np.int)
    for i in range(0, int((prob * 99))):
        rand = random.randint(0, 99)
        while r[rand] == 1:
            rand = random.randint(0, 99)
        r[rand] = 1

    if r[random.randint(0, 99)] == 1:
        return True
    else:
        return False

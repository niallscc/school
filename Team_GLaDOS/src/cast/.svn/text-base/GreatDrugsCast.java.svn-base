...
public class GreatDrugsCast extends Cast {
	/**
	...
	/**
	 * Constructer for the GreatDrugs Cast initializing all of the variables
	 * to their appropriate values.
	 * 
	 * @param caster The caster of this spell.
	 * @param loc The location to cast the spell.
	 */
	public GreatDrugsCast(Being caster, FlatMap loc) {
		if(Server.spellsDEBUG) System.out.println("Great Drugs Cast");
		this.difficulty = 40;
	...
	public void execute(ClientWorldState clientWorldState, int id) {
		...
		if(canCast(caster)){
			...
			if((target != null) && (ManDistance.getDistance(caster.getLocation(), location) <=8)) {
				...
				int healAmount = caster.getIntelligence()*caster.getLevel()/5;
	...
	public CastUpdate execute(ServerWorldState serverWorldState, int id) {
		...
		if(canCast(caster)){
			...
			if((target != null) && (ManDistance.getDistance(caster.getLocation(), location) <=8)) {
				...
				if(Server.spellsDEBUG)System.out.println("Can heal - healing for: " + healAmount);
				//Heal them.
				target.heal(healAmount);
				//if(Server.spellsDEBUG) System.out.println("Heal Done: "+ healAmount);

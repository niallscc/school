...
public class GoodDrugsCast extends Cast {
	/**
	...
	/**
	 * Constructer for the GoodDrugs Cast initializing all of the variables
	 * to their appropriate values.
	 * 
	 * @param caster The caster of this spell.
	 * @param loc The location to cast the spell.
	 */
	public GoodDrugsCast(Being caster, FlatMap loc) {
		if(Server.spellsDEBUG) System.out.println("Good Drugs Cast");
		this.difficulty = 25;
	...
	public void execute(ClientWorldState clientWorldState, int id) {
		...
		if(canCast(caster)){
			...
			//Make sure there is a being, and that they are within 5 manhatten cell distances away
			if((target != null) && (ManDistance.getDistance(caster.getLocation(), location) <=5)) {
				//calculate how much to heal.
				Random amount = new Random();
				//Calculate a random number between 1 and 10
				int healAmount = amount.nextInt(10)+1;
